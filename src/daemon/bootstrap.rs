...
//! 0/1. The wrapped `prompt` trusts $LASTEXITCODE only when the just-run
//! pipeline actually CHANGED it (tracked in `$script:TCLastLEC`); otherwise it
//! folds $? — without this a cmdlet run after a failing native command would
//! inherit that command's stale exit code and be mis-flagged failed.
...
function global:prompt {
  ...
  # $LASTEXITCODE is set ONLY by native commands and then persists unchanged
  # across every later cmdlet, so trusting it whenever non-null makes each
  # cmdlet inherit the previous native command's code (ls after a failed ping
  # would report ping's exit and draw its own failure chrome). Trust $lec only
  # when this pipeline actually CHANGED it; otherwise fold $? to 0/1. Residual:
  # a native command repeating the previous native code reads as "unchanged"
  # and folds to $? (still correctly FAILED, only the exact code differs).
  if ($null -ne $lec -and $lec -ne $script:TCLastLEC) {
    $e = $lec
    $script:TCLastLEC = $lec
  } else {
    $e = if ($q) { 0 } else { 1 }
  }
...
///   - the per-prompt / per-command external utilities the hooks depend on
///     (`od`, `tr` in the hex emitter; `sleep` in the pre-hook drain) are
///     invoked via `command -p` (Warp-tier1): that resolves against the
///     POSIX default PATH, so a user's rc that clobbers or shadows `$PATH`
///     (or drops a `tr`/`sleep` earlier on it) can neither silently break our
///     block hooks nor spoof them with arbitrary code every prompt. `command`
///     is a POSIX shell builtin and this template is only ever run by bash
///     (WSL + ssh both `exec bash`), so `-p` is always available; the utilities
///     are POSIX-standard and thus always found on the default PATH. Pure shell
///     builtins (`printf`, `history`, parameter expansion) need no guard.
///
...
mod tests {
    ...
    fn bash_template_goldens() {
        ...
        // PATH-shadow hardening (Warp-tier1): every per-prompt/per-command
        // external the hooks depend on is called via `command -p` so a
        // clobbered/shadowed $PATH cannot break or spoof our block hooks. The
        // hex emitter's `od`/`tr` and the pre-hook drain's `sleep` are the only
        // externals; everything else is a shell builtin. Guard against a
        // regression that reintroduces a bare call.
        assert!(body.contains("command -p od -v -An -tx1 | command -p tr -d ' \\n'"));
        assert!(body.contains("command -p sleep 0.015"));
        assert!(!body.contains(" od -v -An -tx1 | tr "), "bare od|tr regressed");
    ...
    /// Fix B pin (cmdlet exit-code inheritance): the wrapped `prompt` trusts
    /// $LASTEXITCODE only when this pipeline CHANGED it (vs `$script:TCLastLEC`,
    /// seeded null), else folds $? to 0/1 — so a cmdlet after a failing native
    /// command no longer inherits the stale native exit code. A future revert
    /// to the unconditional `if ($null -ne $lec) { $lec }` trips this.
    #[test]
    fn ps_prompt_exit_attribution_delta() {
        let body = TEMPLATE
            .replace("{BANNER}", "")
            .replace("{TOKEN}", "0123456789abcdef");
        // The delta guard and the $?-fold are both present.
        assert!(body.contains("$script:TCLastLEC = $null"));
        assert!(body.contains(
            "if ($null -ne $lec -and $lec -ne $script:TCLastLEC) {"
        ));
        assert!(body.contains("$script:TCLastLEC = $lec"));
        assert!(body.contains("$e = if ($q) { 0 } else { 1 }"));
        // The old unconditional-trust form is gone (that was the bug).
        assert!(
            !body.contains("$e = if ($null -ne $lec) { $lec } elseif ($q)"),
            "unconditional $LASTEXITCODE trust reintroduces the inheritance bug"
        );
        // Seeding precedes the prompt that reads it.
        let seed_at = body.find("$script:TCLastLEC = $null").unwrap();
        let use_at = body.find("$lec -ne $script:TCLastLEC").unwrap();
        assert!(seed_at < use_at, "TCLastLEC seeded before first prompt use");
    }
